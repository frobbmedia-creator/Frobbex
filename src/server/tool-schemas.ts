...
export const computerClickInput = z.object({ observationId, pid, windowId, elementIndex: z.number().int().nonnegative(), x: z.number().nonnegative().optional(), y: z.number().nonnegative().optional() }).strict().refine((value) => value.x === undefined && value.y === undefined, "Coordinate clicks are not permitted; use an observed elementIndex");
export const computerTypeInput = z.object({ observationId, pid, windowId, elementIndex: z.number().int().nonnegative(), text: z.string().max(20_000) }).strict();
...
export const consequentialAction = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("computer_type"), observationId, pid, windowId, elementIndex: z.number().int().nonnegative(), text: z.string().max(20_000) }).strict(),
]);
